...
**❌ 移除的冗余域名：**
- ChatGPT: 移除 `auth0.com`、`stripe.com`、`sentry.io`、`intercom.io` 等第三方服务域名
- Gemini: 移除 `google.com`、`googleapis.com`、`gstatic.com` 等可能影响其他 Google 服务的域名
- 流媒体: 拆分为独立服务，不再打包

> **为什么要精简？** 原版本域名太多，一些共用域名（如 `auth0.com`、`stripe.com`）被劫持后可能影响其他网站的正常访问，导致节点断流或超时。精简后只劫持各服务的核心域名，大幅减少副作用。
...
- **修复 DNS 缓存配置**：将全局 `dns.disable_cache` 改为 `false`，仅在解锁域名的 DNS 规则中保持 `disable_cache: true`
- **性能提升**：修复后普通域名的 DNS 查询使用缓存，显著降低延迟（从 130-400ms 降回 70ms 左右）
- **保持解锁功能**：解锁域名仍然禁用缓存，确保始终获取解锁机返回的最新 IP
- **优化原理**：普通流量使用 DNS 缓存提高速度，解锁流量禁用缓存确保准确性

> **工作原理**：普通域名使用 DNS 缓存（快速）→ 解锁域名禁用缓存并查询解锁机（返回解锁机 IP）→ 流量通过 `direct` 出站到达解锁机 IP → SNI 代理转发到实际目的地
>
> **对现有用户的影响**：如果你在 v4.1 遇到延迟显著增加的问题，请重新运行脚本并更新配置。新配置恢复了正常速度同时保持解锁功能。
...
>
> **注意**：v4.1 存在全局 DNS 缓存禁用导致的高延迟问题，已在 v4.2 修复。
