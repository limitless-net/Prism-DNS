#!/bin/bash
# ==========================================================
#   V2bX 专用解锁服务总控脚本 (V14.0 官方镜像版)
#   
#   核心修复：
#   1. 废弃 Alpine 现场构建，改用支持 UDNS 的官方 Sniproxy 镜像
#   2. 解决 "Only socket address backends" 报错
#   3. 保持 V2bX JSON 生成与审计规则
#   4. 保持端口暴力清理机制
# ==========================================================
...
force_cleanup() {
    ...
    if command -v docker &> /dev/null; then
        ...
        docker compose -f "$WORK_DIR/docker-compose.yml" down --remove-orphans 2>/dev/null
    fi
...
select_services_logic() {
    ...
    mkdir -p "$WORK_DIR"
    echo "# Dnsmasq Rules" > "$WORK_DIR/dnsmasq.conf"
...
EOF
        ...
        # 核心修改：使用 jpillora/dnsmasq 和 ghcr.io/dlundquist/sniproxy
        # 这两个镜像是经过社区验证的，支持所有特性，不会出现 Alpine 的缺失库问题
...
services:
  ...
  sniproxy:
    image: ghcr.io/dlundquist/sniproxy:latest
    container_name: sniproxy_unlock
    restart: always
    network_mode: host
    privileged: true
    volumes:
...
check_status() {
    ...
    if [ "$DEPLOY_MODE" == "docker" ]; then
        if docker ps | grep -q "dns_unlock"; then echo -e "Dnsmasq: ${GREEN}运行中${NC}"; else echo -e "Dnsmasq: ${RED}停止${NC}"; fi
        if docker ps | grep -q "sniproxy_unlock"; then echo -e "Sniproxy: ${GREEN}运行中${NC}"; else echo -e "Sniproxy: ${RED}停止${NC}"; fi
    else
...
view_logs() {
    ...
    if [ "$DEPLOY_MODE" == "docker" ]; then
        echo "--- Dnsmasq ---"
        docker logs dns_unlock | tail -n 5
        echo ""
        echo "--- Sniproxy ---"
        docker logs sniproxy_unlock | tail -n 5
    else
        echo "--- Dnsmasq Log ---"
...
while true; do
    clear
    echo -e "${SKY}==================================================${NC}"
    echo -e "${SKY}  V2bX 专用解锁服务总控 (V14.0 官方镜像版)${NC}"
    echo -e "${SKY}==================================================${NC}\n"
