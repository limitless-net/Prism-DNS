...
install_base_tools() {
    if ! command -v netstat &> /dev/null || ! command -v dig &> /dev/null || ! command -v lsof &> /dev/null || ! command -v ipset &> /dev/null; then
        echo -e "${YELLOW}>>> 补全基础工具...${NC}"
        if [ -f /etc/debian_version ]; then
            apt-get update -y && apt-get install -y net-tools dnsutils lsof procps iptables ipset
        elif [ -f /etc/redhat-release ]; then
            yum install -y net-tools bind-utils lsof iptables-services ipset
        fi
    fi
}
...
validate_ip() {
    ...
    # IPv6 验证 (严谨匹配)
    if [[ "$ip" =~ ^([0-9a-fA-F]{0,4}:){1,7}[0-9a-fA-F]{0,4}$ ]]; then echo "v6"; return 0; fi
    return 1
}
...
apply_firewall() {
    ...
    # 1. 检查 firewalld 是否干扰 (自动停止)
    if systemctl is-active --quiet firewalld; then
        echo -e "${YELLOW}检测到 firewalld 运行中，正在停止以防止规则冲突...${NC}"
    ...
    if command -v ufw &> /dev/null && systemctl is-active --quiet ufw; then
        ...
        for ip in $ips; do
            ...
            if [ -n "$ip_type" ]; then
                ufw insert 1 allow from "$ip" to any port 53 >/dev/null
                ufw insert 1 allow from "$ip" to any port 80 >/dev/null
                ufw insert 1 allow from "$ip" to any port 443 >/dev/null
                echo -e "放行: ${GREEN}$ip${NC}"
            fi
        done
        
        # 兜底拒绝策略
        ufw deny 53/tcp >/dev/null
        ufw deny 53/udp >/dev/null
        ufw deny 80/tcp >/dev/null
        ufw deny 443/tcp >/dev/null
        ufw reload >/dev/null
        
    else
        # 3. Iptables 模式 (集成 ipset)
        echo -e "模式: Iptables (v4 & v6)"
        
        local use_ipset=0
        if command -v ipset &> /dev/null; then
            use_ipset=1
            echo -e "优化: 启用 ipset 集合匹配"
            # 创建集合 (如果不存在则创建，flush清空)
            ipset create unlock_white hash:ip 2>/dev/null || true
            ipset flush unlock_white
            ipset create unlock_white6 hash:ip family inet6 2>/dev/null || true
            ipset flush unlock_white6

            for ip in $ips; do
                ip_type=$(validate_ip "$ip")
                if [ "$ip_type" == "v4" ]; then
                    ipset add unlock_white "$ip" 2>/dev/null
                elif [ "$ip_type" == "v6" ]; then
                    ipset add unlock_white6 "$ip" 2>/dev/null
                fi
            done
        fi
        ...
        if [ "$use_ipset" -eq 1 ]; then
             iptables -A UNLOCK_FW -m set --match-set unlock_white src -j ACCEPT
        else
             for ip in $ips; do
                 if [ "$(validate_ip "$ip")" == "v4" ]; then
        ...
        if command -v ip6tables &> /dev/null; then
            ...
            if [ "$use_ipset" -eq 1 ]; then
                 ip6tables -A UNLOCK_FW6 -m set --match-set unlock_white6 src -j ACCEPT
            else
                 for ip in $ips; do
                     if [ "$(validate_ip "$ip")" == "v6" ]; then
    ...
    while true; do
        ...
        case "$wl_choice" in
            1)
                ...
                clean_whitelist_file
                read -p "是否立即应用规则? (y/n): " apply_now
                if [[ "$apply_now" == "y" ]]; then apply_firewall; fi
                read -p "按回车继续..."
                ;;
            2)
...
run_install() {
    ...
    if [ -n "$SSH_CLIENT" ]; then
        ...
        if [ -n "$(validate_ip "$CURRENT_SSH_IP")" ]; then
            if ! grep -q "^$CURRENT_SSH_IP$" "$WHITELIST_FILE"; then
                echo "$CURRENT_SSH_IP" >> "$WHITELIST_FILE"
                clean_whitelist_file
                echo -e "${GREEN}安全提示: 已自动将当前 SSH IP ($CURRENT_SSH_IP) 加入白名单${NC}"
            fi
        fi
    fi
