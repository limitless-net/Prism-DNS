#!/bin/bash
# ==========================================================
#   V2bX 专用解锁服务总控脚本 (V19.0 完美运营版)
#   
#   更新日志：
#   1. [新增] "修改解锁规则"：无需卸载，重选服务立即生效
#   2. [新增] "重启服务"：一键重启 Docker 或原生服务
#   3. [新增] "实时流量监控"：查看谁(IP)在访问什么(域名)
#   4. [优化] 状态检测与日志显示的逻辑
# ==========================================================
...
while true; do
    clear
    echo -e "${SKY}==================================================${NC}"
    echo -e "${SKY}  V2bX 专用解锁服务总控 (V19.0 完美运营版)${NC}"
    echo -e "${SKY}==================================================${NC}\n"
